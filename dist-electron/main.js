...
const loudness$1 = /* @__PURE__ */ getDefaultExportFromCjs(loudness);
...
ipcMain.handle("get-volume", async () => {
  return await loudness$1.getVolume();
});
ipcMain.handle("set-volume", async (_, volume) => {
  await loudness$1.setVolume(volume);
});
ipcMain.handle("mute", async () => {
  await loudness$1.setMuted(true);
});
ipcMain.handle("unmute", async () => {
  await loudness$1.setMuted(false);
});
ipcMain.handle("add-profile", (_, profile) => {
...
ipcMain.handle("set-active-profile", (_, id) => {
...
ipcMain.handle("get-muted", async () => {
  return await loudness$1.getMuted();
});
